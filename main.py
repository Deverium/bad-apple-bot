...
TIMEOUT = 0.125/((int(CLIP_FRAMES/4)+1)/CLIP_LENGTH)*18 # 0.125/(((6571/4)+1)/219.0666)*18
        ...
        while i < len(frames)-1:
            ...
            while not disp:
                ...
                if (newTimestamp - oldTimestamp) >= TIMEOUT:
                    ...
                    if LASTMESSAGE:
                        await LASTMESSAGE.edit(content=frames[int(i)])
                    else:
                        LASTMESSAGE = await message.channel.send(frames[int(i)])
